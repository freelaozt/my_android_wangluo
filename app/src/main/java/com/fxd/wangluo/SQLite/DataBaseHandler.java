package com.fxd.wangluo.SQLite;
...
//   查询的数据放入Bean中
    ...
    public void insterOneData(int id, String value) {
        SQLiteDatabase db = this.getWritableDatabase();
        // update Orders set OrderPrice = 800 where Id = 6
        ContentValues cv = new ContentValues();
        cv.put(KEY_LABEL, value);
        db.update(TABLE_NAME, cv, KEY_USER_ID+" = ?",
    ...
    //更新单条数据
    public void update2Data(int id, String value) {
        SQLiteDatabase db = this.getWritableDatabase();
        // update Orders set OrderPrice = 800 where Id = 6
        ContentValues cv = new ContentValues();
        cv.put(KEY_HEAD_IMG, value);
        db.update(TABLE_NAME, cv, KEY_USER_ID+" = ?",
                new String[]{String.valueOf(id)});
        db.close();
    }
